"""
Copy from Allegro https://github.com/mir-group/allegro/blob/22f673c565d148ae8e9394443baa9f5b5716c8e7/allegro/nn/_strided/_linear.py
Copy from Allegro https://github.com/mir-group/allegro/blob/22f673c565d148ae8e9394443baa9f5b5716c8e7/allegro/nn/_strided/_layout.py
...
class StridedLayout(torch.nn.Module):
    ...
    def __init__(self, irreps: Irreps, pad_to_multiple: int = 1):
        ...
        self.base_dim = int(
            math.ceil(self.base_irreps.dim / self.pad_to_multiple) * self.pad_to_multiple
        )
        ...
        for mul_i in range(self.mul):
            for irrep_i, (_, irrep) in enumerate(self.base_irreps):
                ...
                catted_indexes = (
                    torch.arange(irrep.dim) + self.irreps[:irrep_i].dim + irrep.dim * mul_i
                )
        ...
        assert torch.all(
            self.indexes_to_strided[self.indexes_to_catted] == torch.arange(self.irreps.dim)
        )
...
def codegen_strided_linear(
    ...
    ins_group_irrep_slice: list[Union[tuple[int, int], Any]] = []
    ...
    _outs: list[list] = [[] for _ in range(len(layout_out.base_irreps))]
    ...
    for ins_grp_i, (ins_grp, ins_grp_ins) in enumerate(zip(ins_per_output, ins_group_irrep_slice)):
        ...
        _outs[ins_grp[0].i_out].append(torch.einsum(f"{z}vun,zuni->zvi", this_w, to_mix))
        w_index += n_weight
    ...
    outs: list = [
        ...
        for i, (ins_grp, o) in enumerate(zip(ins_per_output, _outs))
    ]
    outs = [
        (
...
def Linear(
    ...
    _instructions = [Instruction(i_in=e[0], i_out=e[1], path_shape=None) for e in instructions]
...
class LinearWrapper(torch.nn.Module):
    ...
    def to(self, *args, **kwargs):
        self.linear.to(*args, **kwargs)

    def extra_repr(self):
        return super().extra_repr() + f" params: {self.linear.weight_numel}"
